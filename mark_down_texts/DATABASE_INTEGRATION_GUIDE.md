...
For internal systems without authentication, use permissive RLS policies:

### Inventory Table - Unrestricted Access
...
### Requests Table - Unrestricted Access

```sql
CREATE POLICY "Allow all access to requests" ON requests
FOR ALL
USING (true)
WITH CHECK (true);
```

**Note:** These policies allow full unrestricted access (SELECT, INSERT, UPDATE, DELETE) since this is an internal system without user authentication. If you need to restrict access later, you can update these policies.
