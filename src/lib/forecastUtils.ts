...
export function getLinearPredictions(data: number[], stepsAhead: number = 3) {
  if (data.length < 2) return { predicted: [], slope: 0, intercept: 0, rSquared: 0, line: (x: number) => 0 };
...
export function getPolynomialPredictions(data: number[], stepsAhead: number = 3, degree: number = 2) {
...
export function buildForecast(data: number[], stepsAhead: number = 3) {
  ...
  return {
    ...
    trend: linear.slope > 1 ? 'rising' : linear.slope < -1 ? 'falling' : 'stable'
  };
}

// ── INVENTORY STATS (simple-statistics) ────────────────────────────────────
export function computeInventoryStats(burnRates: number[], stockLevels: number[]) {
  ...
  return {
    ...
    stockBurnCorrelation: burnRates.length > 1
...
export function buildProcurementRows(consumableBurnRate: any[]) {
  return consumableBurnRate.map(item => {
    ...
    const suggestedOrder = Math.max(0, safetyStock + predDemand - item.currentStock);
    const weeksLeft = item.weeklyBurnRate > 0
